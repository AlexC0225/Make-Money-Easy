...
function describeStrategy(strategyName?: string) {
  switch (strategyName) {
    case 'hybrid_tw_strategy':
      return '用 MA20、MA60 與 Best Four Point 條件判斷趨勢切入與出場。'
    case 'connors_rsi2_long':
      return '用 SMA200 與 RSI(2) 尋找超跌反彈，偏向短波段操作。'
    case 'tw_momentum_breakout_long':
      return '用趨勢、突破、量能與 RSI 一起確認強勢動能進場。'
    default:
      return '依策略條件在指定區間內重建交易，評估報酬、回撤與持倉表現。'
  }
}
  ...
  return (
    <div className="page-grid">
      <section className="hero-strip">
        <div>
          <p className="hero-kicker">Backtest Lab</p>
          <h2>回測分析</h2>
          <p>這一頁只專注在回測設定、績效摘要、圖表與回測紀錄，避免和即時策略訊號混在一起。</p>
        </div>
      </section>
      ...
      <div className="backtest-layout">
        <Panel title="回測設定" subtitle="Manual Backtest">
          <div className="stack-form">
            <div className="backtest-form-grid">
              <label>
                ...
                <textarea
                  ...
                  placeholder="例如 2330, 2317, 2454；留空時會使用系統預設清單"
                />
              </label>
              ...
              <label>
                ...
                >
                  <option value="fixed_shares">固定張數</option>
                  <option value="cash_percent">資金百分比</option>
                </select>
              </label>
              ...
              <label>
                ...
                <input
                  ...
                  onWheel={(event) => event.currentTarget.blur()}
                  onChange={(event) => setForm((current) => ({ ...current, max_open_positions: Number(event.target.value) }))}
                />
              </label>
            </div>
            ...
            {defaultTargetsQuery.error ? <p className="error-text">{defaultTargetsQuery.error.message}</p> : null}
          ...
          {highlightedResult ? (
            <div className="stack-form">
              <div className="info-card">
                ...
                {highlightedResult.is_portfolio ? <p>股票池：{highlightedResult.portfolio_codes.join(', ')}</p> : null}
                <p>最多持倉：{highlightedResult.result.max_open_positions ?? form.max_open_positions}</p>
              </div>
              ...
              <div className="setup-summary-grid">
                ...
                <article className="setup-summary-card">
                  ...
                  <p>{highlightedResult.is_portfolio ? '回測結束時仍持有的股票檔數。' : '回測結束時仍持有的股數。'}</p>
                </article>
              </div>
            </div>
          ) : (
            <div className="empty-card">先執行一次回測，這裡才會顯示回測摘要。</div>
          )}
        </Panel>
      </div>
        ...
        {highlightedResult ? (
          <div className="stack-form">
            {highlightedResult.is_portfolio ? (
              ...
              <>
                {highlightedHistoryQuery.isPending ? <p className="muted-text">正在載入回測圖表資料...</p> : null}
          ...
          <div className="empty-card">先執行一次回測，這裡才會顯示圖表。</div>
        )}
      </Panel>
      ...
      <Panel title="交易明細" subtitle="Trade Ledger">
        {highlightedResult ? (
          highlightedResult.result.trades.length > 0 ? (
            <div className="stack-form">
              <p className="muted-text">共 {highlightedResult.result.trades.length} 筆回測交易，以下依時間由新到舊排列。</p>
